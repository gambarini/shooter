...
| **the arena layout is a pure function of the wave** | a `Math.random()` in the layout — which no screenshot can see, and which would break `--seed`, the `ab` baseline and item 43's seeded daily — plus a rotation that hands a plain wave the boss floor, or drifts off the cover budget, anywhere in its 25-wave period, plus a `SYM` that has stopped re-orienting anything. That last one is measured two ways, because a height profile confounds it either way and no single pair escapes both. Within the rotation, across waves that share an archetype AND a height profile (item 96): consecutive waves differ in profile whatever SYM does, so the consecutive pair this used to compare reported a moved floor on a build where SYM was eight identities. Across the 25-wave wrap, where no wave shares wave 1's profile, by comparing FOOTPRINTS with the height dropped (item 97) — `prof` alone writes the height, so what is left is the field SYM writes. Both go red on a build with SYM replaced by eight identities; before item 97 the wrap check stayed green |
| **the boss floor is a different floor each time** | an archetype whose box set is symmetric enough that SYM only permutes slot order — which is what OPEN did until item 87, fighting every boss and every FRENZY on one picture inside a suite that reported five different signatures for it. Compared over the boss floor's own 40-wave period, and compared as pictures — as is every other check in that scenario that claims a floor moved (item 94). Which comparisons take a picture and which take the raw signature is `pic()`'s own comment in `arena.mjs` |
...
- **sample and act in ONE eval, never across a round trip** (item 90). All three
  ...
  check went red on a commit that had changed nothing. Reading and acting in the same
  JavaScript turn removes the clock from the check, and the retry loop that surrounds it
