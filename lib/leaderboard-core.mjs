...
//
// Moved here from netlify/lib/ in roadmap item 52. Deliberately free of any
// platform API (Workers, Node, network) so it can be unit-tested with plain
// `node --test lib/leaderboard-core.test.mjs` — which is exactly what item 53's
// GitHub Actions workflow runs as a deploy gate. functions/api/scores.js is the
// thin I/O shell around this.
//
// What used to live here and does NOT any more: `insertRun`, `rankOf`, `topN`
// and `board`. Those were array manipulation standing in for a database — the
// leaderboard now lives in D1, so insertion is an INSERT, ranking is an indexed
// `count(*) WHERE score > ?`, and the board is a LIMIT query. `MAX_RUNS` went
// with them: the 100k cap only existed so a single JSON blob couldn't grow
// without bound, which is not a constraint a table has.

export const MAX_NAME = 12;
export const MAX_SCORE = 100_000_000; // sanity ceiling; the real check is plausibleRun
export const MAX_WAVE = 10_000;
export const TOP_N = 10;

// Strip control chars and angle brackets (a belt against HTML injection in the
// rendered table — full escaping still happens at render, see escapeHtml in
// index.html), collapse whitespace, clamp length. Empty -> "ANON".
...
// — the handler stamps created_at so this stays pure/deterministic for tests.
